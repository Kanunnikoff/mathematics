/*
 * @(#)TrimorphicNumber.java        1.0 17.02.2013
 *
...
/**
 * Триморфное число — число, десятичная запись куба которого оканчивается цифрами самого этого числа.
 *
 * @author Dmitry Tsechoev
 * @version 1.0 17.02.2013
 */
public class TrimorphicNumber implements NumericalSequence {
    /**
     * Создаёт новый объект для рассчёта триморфных чисел.
     */
    private TrimorphicNumber() {
    }

    /**
     * Возвращает триморфное число для целочисленного значения n.
     *
     * @param n целое неотрицательное число (n>=0), для которого требуется вычислить триморфное число.
     * @return триморфное число под номером n.
     */
    public static long get(long n) {
        long limit = (long) Math.cbrt(Long.MAX_VALUE);
        long result = -1;
        long count = 0;

        for(long i = 0; i <= limit; i++) {
            if(Long.toString(i * i * i).endsWith(Long.toString(i))) {
                count++;
                if(count == n) {
                    result = i;
                    break;
